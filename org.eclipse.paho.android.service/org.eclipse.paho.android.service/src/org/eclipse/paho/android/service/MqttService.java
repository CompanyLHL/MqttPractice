...
public class MqttService extends Service implements MqttTraceHandler {
	...
	private MqttConnection getConnection(String clientHandle) {
		// System.out.println("the cliecnthandle is===>" + clientHandle);
	...
	private void bindService() {
		...
		Notification myNotify = new Notification.Builder(this)
				.setSmallIcon(R.drawable.mqttservice)
				.setTicker("MQTT Service is comming")
				.setContentTitle("MQTT Service").setContentText("Ummmm, clear")
	...
	private class NetworkConnectionIntentReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (isOnline()) {
				...
				// connecting
				preference.setIs_net_broken(false);
				Toast.makeText(context, "网络连接上了！", Toast.LENGTH_LONG).show();
				reconnect();
			} else {
				preference.setIs_net_broken(true);
