...
public class MqttAndroidClient extends BroadcastReceiver implements
	...
	public enum Ack {
		...
		AUTO_ACK,
		/**
		 * When {@link MqttCallback#messageArrived(String, MqttMessage)} returns
		 * the message will not be acknowledged as received, the application
		 * will have to make an acknowledgment call to {@link MqttAndroidClient}
		 * using {@link MqttAndroidClient#acknowledgeMessage(String)}
		 */
		MANUAL_ACK
	}
...
//	private volatile boolean unregisterReceiver = true;
	...
	public void close() {
		if (clientHandle == null) {
			clientHandle = mqttService.getClient(serverURI, clientId,
					String.valueOf(myContext.hashCode()), persistence);
		}
	 ...
	 */
	Intent serviceStartIntent = null;

	@Override
	public IMqttToken connect(MqttConnectOptions options, Object userContext,
			IMqttActionListener callback) throws MqttException {

		// check to see if there is a network connection where we can send data
		// before attempting the connect
		ConnectivityManager conManager = (ConnectivityManager) myContext
...
//		unregisterReceiver(this);
	...
	public IMqttDeliveryToken publish(String topic, byte[] payload, int qos,
		...
		MqttDeliveryTokenAndroid token = new MqttDeliveryTokenAndroid(this,
				userContext, callback, message);
		String activityToken = storeToken(token);
	 ...
	 *            one specified on an earlier subscribe.
	 * @return token used to track and wait for the unsubscribe to complete. The
	 *         token will be passed to callback methods if set.
	 * @throws MqttException
	 *             if there was an error unregistering the subscription.
	...
	public void onReceive(Context context, Intent intent) {
		...
		// }
			...
			mqttService.traceError(MqttService.TAG,
					"Callback action doesn't exist.");
		}
	...
	public boolean acknowledgeMessage(String messageId) {
		if (messageAck == Ack.MANUAL_ACK) {
			Status status = mqttService.acknowledgeMessageArrival(clientHandle,
					messageId);
	...
	private void traceAction(Bundle data) {
		...
		if (traceCallback != null) {
			String severity = data
					.getString(MqttServiceConstants.CALLBACK_TRACE_SEVERITY);
			String message = data
					.getString(MqttServiceConstants.CALLBACK_ERROR_MESSAGE);
			String tag = data
					.getString(MqttServiceConstants.CALLBACK_TRACE_TAG);
